...
extern void debug_mem_reset(void); /* debug_mem_reset allows you to clear all memory stored in the debugging system if you only want to record allocations after a specific point in your code*/
extern bool debug_memory(void); /* debug_memory checks if any of the bounds of any allocation has been over written and reports where to standard out. The function returns TRUE if any error was found*/
...
These are very fast pseudo random number generators useful for graphics applications. NOT cryptographically safe in any way.
*/
...
extern void math_project2d(float *output, float *plane_pos, float *normal, float *pos, float *vector);
/* projects fr0m aposition along a vector on to a positioned plane in 3D for 32 bit floats.*/
extern void math_project3d(float *output, float *plane_pos, float *normal, float *pos, float *vector);
/* Computes the intersection between two lines in 2D for 32 bit floats.*/
extern void math_intersect2d(float *output, float *line_a0, float *line_a1, float *line_b0, float *line_b1);
...
extern void math_transform4d(float *output, const float *matrix, const float x, const float y, const float z, const double w);
...
extern void math_matrix_to_pos_quaternion_scale(float *matrix, float *pos, float *quaternion, float *scale);
/* Converts a 32 bit float position, scale and quaternion to a 4x4 32 bit float matrix.*/
extern void math_pos_quaternion_scale_to_matrix(float *pos, float *quaternion, float *scale, float *matrix);
